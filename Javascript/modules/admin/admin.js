...
const Admin = {
    ...
    async saveFolder() {
        ...
        try {
            const { data, error } = await db.createFolder(name, parentId, Sidebar.currentCategory);
    ...
    async savePage() {
        ...
        try {
            const { data, error } = await db.createPage(
    folderId,
    title,
    description,
    '',
...
bindImageUpload() {
    ...
    addImageBtn.addEventListener('click', () => {
        if (!Content.currentPage) {
            Layout.showToast('Pilih halaman dulu', 'error');
            return;
        }
        imageInput.click();
    });
    ...
    imageInput.addEventListener('change', async (e) => {
        ...
        try {
            Layout.showToast('Uploading image...', 'info');

            const imageUrl = await uploadImageToCloudinary(file);

            const { error } = await db.updatePage(
                Content.currentPage.id,
                { image_url: imageUrl }
            );

            if (error) throw error;

            Layout.showToast('Image berhasil diupload!', 'success');
            await Content.refresh();

        } catch (err) {
            console.error(err);
            Layout.showToast('Upload gagal', 'error');
        }
    });
},
...
const CLOUDINARY_URL = "https://api.cloudinary.com/v1_1/dwufjtb7i/image/upload";
const CLOUDINARY_PRESET = "web-notion-upload";
